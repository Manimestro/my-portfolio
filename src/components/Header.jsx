import React from 'react'
import {motion } from 'framer-motion';
import {styles} from '../style';
import { ComputersCanvas } from './canvas';
const Header = () => {
  return (
    <section
    className="relative w-full h-screen mx-auto"
    >
      <div className={`${styles.paddingX} absolute
      inset-0 top-[120px] max-w-7xl 
      max-auto flex flex-row items-start gap-5`}>
        <div className='flex flex-col 
        justify-center items-center mt-5'>
          <div className=' w-5 h-5 bg-[#915eff] rounded-full'/>
            <div className='w-1 sm:h-80 h-40 violet-gradient  ' />
        </div>
        <div>
              <h1 className={`${styles.heroHeadText}`}>Hi, I'm <span className='text-[#915eff] '>
                Manikanta</span></h1>
                <p className={`${styles.heroSubText} mt-2
                text-white-100`}>
                  I develop Web Applications,<br 
                  className='md:hidden block' /> And user interfaces
                </p>
            </div>
       
      </div>
      <ComputersCanvas/>
      <div className="absolute xs:bottom-28 
      md:bottom-16
      w-full flex justify-center items-start p-2"> 
        <a
        href='#about'>
          <div className='mt-4 w-[35px] h-[64px] rounded-3xl
          border-4 border-secondary flex justify-center 
          items-start p-2 -mb-14' ></div>
          
          <motion.div
          animate = {{
            y : [0,36,0],
            
          }}
          transition={{
            duration : 1.5 ,
            repeat :Infinity,
            repeatType :'loop',
            
          }    }
          className =" ml-3 w-3 h-3 rounded-full bg-secondary "
        />
        
        </a>
      

      </div>

    </section>
  )
}

export default Header